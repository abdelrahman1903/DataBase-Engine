...
//            i--;
            ...
            for (int c = i; c < pa.getTuples().size(); c++) {
                if (c == Table.MaximumRowsCountinPage-1) {
...
//                System.out.println("size = "+ p.getTuples().size());
                for (int y = 0; y < p.getTuples().size(); y++) {
                    if (y == Table.MaximumRowsCountinPage-1) {
...
//            throw new DBAppException("Missing clustering key");
        ...
        if(chackForIndex(strTableName,currTable.getStrClusteringKeyColum())){
            ...
            BTree index = indexDeserialization(indexName);
//            System.out.println("my key ______" + htblColNameValue.get(currTable.getStrClusteringKeyColum()) );
            System.out.println("my value ______" + value );
...
//                    Vector<String> keys = new Vector<>();
            ...
            if(indexExists && pagesToSearch!=null) {
                for (int pagen = 0; pagen < pagesToSearch.size(); pagen++) {
                    String pageName = table.getPages().get(pagesToSearch.get(pagen)-1);
                    Page p = table.pageDeserialization(pageName);
                    for (int i = 0; i < p.getTuples().size(); i++) {
                        Tuple t = p.getTuples().get(i);
...
//		                    Vector<String> keys = new Vector<>();
            ...
            if(chackForIndex(tableName, strColumnName)) {
                ...
                Table currTable = DeserializeTable(tableName);
                for (int j = 0; pagesHavingNeededRecords != null&&j < pagesHavingNeededRecords.size(); j++) {
...
//        htblColNameValue.put("Sname", new String("Ahmed" ) );
...
//
//        htblColNameValue.clear( );
//        htblColNameValue.put("SID", new Integer( 20 ));
...
//        htblColNameValue.clear( );
//        htblColNameValue.put("Major", new String("met" ) );
//        htblColNameValue.put("SGPA", new Double( 1.1 ) );
//        dbApp.updateTable( strTableName3 , "17" , htblColNameValue );
...
////            pageSerialization(page1);
        ...
        arrSQLTerms[0]._strTableName = strTableName3;
        arrSQLTerms[0]._strColumnName= "SID";
        arrSQLTerms[0]._strOperator = "=";
        arrSQLTerms[0]._objValue = 19;

//        arrSQLTerms[1] = new SQLTerm();
//        arrSQLTerms[1]._strTableName = strTableName3;
//        arrSQLTerms[1]._strColumnName= "Sname";
//        arrSQLTerms[1]._strOperator = "=";
//        arrSQLTerms[1]._objValue = "Ola";
//
//        arrSQLTerms[2] = new SQLTerm();
//        arrSQLTerms[2]._strTableName = strTableName3;
//        arrSQLTerms[2]._strColumnName= "SGPA";
//        arrSQLTerms[2]._strOperator = "<";
//        arrSQLTerms[2]._objValue = 3.1;
//
//        arrSQLTerms[3] = new SQLTerm();
//        arrSQLTerms[3]._strTableName = strTableName3;
//        arrSQLTerms[3]._strColumnName= "Major";
//        arrSQLTerms[3]._strOperator = "!=";
//        arrSQLTerms[3]._objValue = "CS";
